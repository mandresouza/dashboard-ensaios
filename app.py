...
# =======================================================================
# [BLOCO 04] - PROCESSAMENTO TÉCNICO (VERSÃO FINAL - ZERO ERROS)
# =======================================================================
...
def processar_ensaio(row, classe_banc20=None):
    """
    Processa uma linha com RIGOR TOTAL e compatibilidade com Gráficos de Posição.
    """
    medidores = []
    ...
    for pos in range(1, tamanho_bancada + 1):
        ...
        # --- COLETA DE ERROS PONTUAIS (O QUE ESTAVA FALTANDO!) ---
        erros_pontuais = []
        ...
        if v_cn is None and v_cp is None and v_ci is None:
            medidores.append({
                ...
                "limite": limite, "erros_pontuais": [] # Lista vazia para não dar KeyError
            })
            continue
        ...
        # Retorno completo com a chave erros_pontuais
        medidores.append({
            "pos": pos, "serie": serie,
            ...
            "erros_pontuais": erros_pontuais # AGORA A OUTRA ABA VAI FUNCIONAR!
        })
...
def renderizar_card(medidor):
    ...
    # A técnica abaixo remove espaços à esquerda que causam o erro de exibir o código como texto
    conteudo_html = f"""
...
def pagina_visao_mensal(df_completo):
    ...
    # =====================================================
    # TABELA DE RESULTADOS CONSOLIDADOS (SUBSTITUÍDA)
    # =====================================================
    st.markdown("---")
    with st.expander("📊 Verificação dos Resultados do Mês (Resultados Consolidados)"):
        if not df_daily.empty:
            # Criamos uma cópia para formatar a data na exibição sem afetar o gráfico
            df_exibicao = df_daily.copy()
            df_exibicao['Data'] = df_exibicao['Data'].dt.strftime('%d/%m/%Y')
            
            # Selecionamos as colunas que compõem os gráficos acima
            st.dataframe(
                df_exibicao[['Data', 'Total', 'Aprovados', 'Reprovados', 'Contra Consumidor', 'Taxa de Aprovação (%)']], 
