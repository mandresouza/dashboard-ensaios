# ===============================================================
# ARQUIVO app.py (VERSÃO FINAL E COMPLETA - "FORÇA BRUTA")
# ===============================================================
...
def main():
    ...
    if not creds and "code" in query_params:
        try:
            with st.spinner("Autenticando com o Google..."):
                flow = create_flow()
                flow.fetch_token(code=query_params.get_all('code')[0])
    ...
    if not creds:
        ...
        try:
            ...
            st.link_button("Fazer Login com o Google e Autorizar", auth_url, use_container_width=True, type="primary")
        except Exception as e:
    ...
    try:
        ...
        df_completo = carregar_dados(creds)
        if df_completo.empty:
            st.warning("Aguardando dados... Se esta mensagem persistir, verifique as permissões e os nomes das abas na sua planilha.")
        else:
        ...
        st.error("Ocorreu um erro ao carregar o dashboard após a autenticação.")
        st.code(traceback.format_exc())
